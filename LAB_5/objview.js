...
function handleKeyDown(event) {
	...
	switch (key) {
		...
		case 72: // ytheta-// h
			dyt -= stept;
			document.getElementById("xrot").value = dyt;
			break;
		case 75: //ytheta+// k
			dyt += stept;
			document.getElementById("xrot").value = dyt;
		...
		case 85: //xtheta+// u
			dxt -= stept;
			document.getElementById("yrot").value = dxt;
			break;
		case 74: //xtheta-// j
			dxt += stept;
			document.getElementById("yrot").value = dxt;
...
function buildModelViewProj() {
	...
	if (changePos == 1) {
		...
		eye = [cx, cy, cz];
		vec3.set(up, localRadius * Math.sin(rthe) * Math.cos(rphi), localRadius * Math.sin(rthe) * Math.sin(rphi),
			localRadius * Math.cos(rthe));
	}
